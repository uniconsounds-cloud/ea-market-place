-- SQL to safeguard non-zero history and allow admin test ports to sync history
-- Run this in your Supabase SQL Editor if you want test ports without a real license to sync history.
...
BEGIN
    ...
    IF NOT v_is_valid AND p_port_number IS NOT NULL THEN
        -- Check if registered as active license
        SELECT EXISTS(
    ...
    IF NOT v_is_valid AND p_port_number IS NOT NULL THEN
        -- Check if registered as Admin Test Port
        SELECT EXISTS(
            SELECT 1 FROM public.admin_test_ports
            WHERE account_number = p_port_number
        ) INTO v_is_valid;
    END IF;
