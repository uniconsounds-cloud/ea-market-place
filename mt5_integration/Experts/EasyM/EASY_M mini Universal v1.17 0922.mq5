...
// - Symbol Activation Switches (11 symbols: 10 existing + AUDNZD replacement) with Close-Only Graceful Shutdown
...
//+------------------------------------------------------------------+
//|  EASY_M v1.0 0121.mq5                       |
//|  Multi-Symbol MVP (5 correlation pairs = 10 symbols)              |
...
#property description "EasyM mini Universal v1.17 0922"
...
input int InpMaxNewSymbolsPerBar = 5; // Max new symbols per bar
input int InpMaxActiveSymbols    = 8; // Max active symbols
input int InpMaxGridAddsPerBar   = 2; // Max order adds per bar
...
input int InpSlowMaxNewSymbolsPerBar = 2; // SLOW: Max new symbols per bar
input int InpSlowMaxActiveSymbols    = 4; // SLOW: Max active symbols
input int InpSlowMaxGridAddsPerBar   = 1; // SLOW: Max order adds per bar
...
input int    InpWorstTopN               = 2;    // Worst symbols count
...
static const string InpEA_Label            = "EASY_M 5Pair"; // Order comment / EA label
...
//-------------------- 5 correlation pairs (10 symbols) --------------
SymbolConfig g_cfg[] =
{
   ...
   // Pair 5: USDCAD vs AUDNZD (Magic: 2009, 2011)
   {"USDCAD", 5, 9, false}, {"AUDNZD", 5, 11, false},

   // --- Legacy Pairs (Magic: 2010 - Close-Only & Auto-Collapse)
   {"AUDCAD", 6, 10, true}
};
...
{
   int baseRows = 5; // 5 pairs = 10 active symbols
   ...
   return baseRows; // Auto-collapse back to 5 rows when all legacy positions close!
}
...
void DashboardBuild()
{
   ...
   DashMakeText(DashNameRC("C_TAG",0,5),t, cy0+tyPad, g_guard_ok ? "=== EasyM mini Universal v1.17 0922 ===":" ", DASH_GOLD, fs, 20020);
...
void DashboardUpdate()
{
   ...
   // Per row data (dynamic)
   int nSym = ArraySize(g_state);
   SymDash sd[];
   ArrayResize(sd, nSym);
   for(int i=0; i<nSym; i++) BuildSymDash(g_state[i], sd[i]);
   ...
   for(int pr=0; pr<bBodyRows; pr++)
   {
      ...
      for(int k=0; k<2; k++)
      {
         ...
         if(idx < 0 || idx >= nSym)
         {
            // Empty slot on last row if odd number of symbols
            ObjectSetString(g_dashChart, DashNameRC("B_SYM", row, k*7+0), OBJPROP_TEXT, "");
            ObjectSetString(g_dashChart, DashNameRC("B_SIDE", row, k*7+1), OBJPROP_TEXT, "");
            ObjectSetString(g_dashChart, DashNameRC("B_CNT", row, k*7+3), OBJPROP_TEXT, "");
            ObjectSetString(g_dashChart, DashNameRC("B_PNLN", row, k*7+5), OBJPROP_TEXT, "");
            ObjectSetString(g_dashChart, DashNameRC("B_LOT", row, k*7+6), OBJPROP_TEXT, "");
            DashHideRect(DashNameRC("B_PNLF", row, k*7+4));
            for(int bi=0; bi<VU_MAX_BARS; bi++) DashHideRect(DashNameRC("VU", row*100 + k*7, bi));
            continue;
         }
         ...
         int mid = bgX + bgW/2;
         int half = MathMax(1, bgW/2 - lenPad);
   ...
   ObjectSetString(g_dashChart, DashNameRC("C_TAG",0,5), OBJPROP_TEXT, g_guard_ok ? "=== EasyM mini Universal v1.17 0922 ===":" ");
//DashMakeRect(DashName("CBG"), cx0, cy0,      cW, rowH, g_guard_ok ? clrDarkRed:clrDarkViolet, 150, 20000);
...
int OnInit()
{
   ...
   Print("EASY_M_MS_5Pairs initialized. Symbols=", n, ", MagicBase=", InpMagicBase);
...
void OnTick()
{
   ...
   */ //============== Disable inside Port Check =================
    ...
    EaezeCheckLicenseAndSync(EA_PRODUCT_ID, "EASYM_MINI", "1.12.8.U2", 20, InpMagicBase, InpMagicBase);
