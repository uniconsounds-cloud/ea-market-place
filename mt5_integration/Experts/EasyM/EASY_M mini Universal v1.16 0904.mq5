// v1.16 (0904) - [2026-09-04] Fixed basket exit by removing InpAvoidMultiExitBar lockout, enabling full-basket close (all orders per side), and ensuring broker filling mode detection on exit.
//+------------------------------------------------------------------+
...
#property description "EasyM mini Universal v1.16 0904"
...
void DashboardBuild()
{
   ...
   DashMakeText(DashNameRC("C_TAG",0,5),t, cy0+tyPad, g_guard_ok ? "=== EasyM mini Universal v1.16 0904 ===":" ", DASH_GOLD, fs, 20020);
...
void DashboardUpdate()
{
   ...
   // Per row data
   SymDash sd[10];
   for(int i=0;i<10;i++) BuildSymDash(g_state[i], sd[i]);

   // max abs pnl for scaling bars
   double maxAbs=0.0;
   for(int i=0;i<10;i++) { double a=MathAbs(sd[i].pnl); if(a>maxAbs) maxAbs=a; }
   ...
   ObjectSetString(g_dashChart, DashNameRC("C_TAG",0,5), OBJPROP_TEXT, g_guard_ok ? "=== EasyM mini Universal v1.16 0904 ===":" ");
//DashMakeRect(DashName("CBG"), cx0, cy0,      cW, rowH, g_guard_ok ? clrDarkRed:clrDarkViolet, 150, 20000);
