...
//|   0416_v102 show Lot and MaxDD and work with EAE_Monitor.mq5      |
//+------------------------------------------------------------------+
#property strict
...
//==================================================================//
// [180] (REMOVED) Safety Gate V1.2.2 - Logic stripped for clean baseline revert.
input int    InpFriLockProfitBefore_Hours = 6;   // Fri Lock Profit Start (Hrs)
input double InpFridayMaxDD             = 10.0;  // Fri Rescue Limit (%)
...
void FridayPlanUpdate()
{
   ...
   // --- [NEW] Friday Net Profit Lock logic ---
   if(activeDay && (g_friState == EZ_STATE_FRI_MANAGE))
   {
      if(ttc <= InpFriLockProfitBefore_Hours * 3600)
      {
         double dayClosedProfit = ClosedProfitSince(DayStart(now));
         double floatingProfit  = BasketProfitByMagicSide(g_magicBuy, POSITION_TYPE_BUY) + 
                                 BasketProfitByMagicSide(g_magicSell, POSITION_TYPE_SELL);
         
         // If we have a loss but it's smaller than today's closed profit => Lock it.
         if(floatingProfit < 0 && MathAbs(floatingProfit) < dayClosedProfit)
         {
            Log(StringFormat("[FRI] Lock Profit triggered: Floating=%.2f, DayClosed=%.2f. Closing all.", floatingProfit, dayClosedProfit));
            if(cntB > 0) CloseAllByMagicSideTagged("LOK", g_magicBuy, POSITION_TYPE_BUY);
            if(cntS > 0) CloseAllByMagicSideTagged("LOK", g_magicSell, POSITION_TYPE_SELL);
         }
      }
   }
...
void OnTimer()
{
   // [FIX] Force ChartRedraw(0) to ensure UI updates when market is closed (no ticks).
   // Throttled to every timer beat (1s) to minimize flicker.
...
int OnInit()
{
   // [EAEZE] Web-based Licensing Integration
