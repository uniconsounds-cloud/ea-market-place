...
//|   0413_v101 1 User sync Farm UI                                   |
//|  v1.40  - NEW: Instant Dashboard (Async Boot), Smart Sync (Delta Hashing), and Zero-Contention Storage.|
//|  1.401 - NEW: Advanced Friday Safety (8-6-4-1 Rule) + Dynamic Rescue Growth Guard.               |
//|  1.402 - NEW: High-Priority Circuit Breaker (18% / 28% DD) + Cooldown.                           |
//|  1.403 - NEW: Market Safety Gates (Velocity, ATR, Spread, News Filter).                          |
//|  1.404 - FIX: Dashboard Scaling offset correctly handles Mac Retina displays.                    |
//|  1.405 - FIX: Supabase Order Sync robust update for Phase 1 Migration.                           |
...
int    G_FriSoftClose_Hours   = 4; // [4h] Start soft liquidation (No rescues)
int    G_FriForceClose_Hours  = 1; // [1h] Hard exit all positions
...
const double G_GATE_ATR_RATIO_LIMIT  = 3.0;    // ATR Ratio Limit (Candle / ATR14)
...
//--- Friday Growth Protection 
double g_friBaseDD        = 0.0;   // DD% snapshot at 8h mark
bool   g_friRescueBlocked = false; // Flag set if DD grows > limit

//--- Circuit Breaker State
bool     g_breakerRescueBlocked = false; // Level 1 flag
datetime g_breakerLevel2Start   = 0;     // Verification timer
datetime g_globalCooldownUntil  = 0;     // 30m pause timer

//--- Market Safety Gates State
string   g_marketGateStatus = "N/A";
bool     g_marketGateBlocked = false;
...
// Helper to get current Float Drawdown %
...
//--- Global state for Background Boot Sequence
enum ENUM_BOOT_STATE
{
   BOOT_STARTING,
   BOOT_CLEANUP,
   BOOT_LICENSE,
...
ENUM_BOOT_STATE g_bootState = BOOT_STARTING;

// Helper to update status on Dashboard Title bar
void UpdateBootStatus(string msg)
{
   g_eae_sync_status = "[BOOT: " + msg + "]";
}
...
enum ENUM_FRIDAY_RULE
{
   EZ_RULE_NONE = 0,               // Disabled
   EZ_RULE_FIXED_8_6_4_1 = 1,      // Standard 8/6/4/1 Rule
   EZ_RULE_DYNAMIC_RESCUE = 2      // Rescue-based Limit
};
...
// Safety Status (Baseline)
bool   g_isMarketSafe      = true;
string g_safetyReason      = "OK";
bool   g_isFridayDDLimit   = false;
double g_fridayNet         = 0.0;
int    g_cooldownSec       = 0;
...
// [400] (REMOVED) SAFETY GATES Logic - Stripped for clean baseline revert.
...
// [NEW] V1.408: Count only Rescue/Entry orders (Excluding Follow)
int CountRescuePositionsByMagicSide(const int magic, const ENUM_POSITION_TYPE side)
{
   int total = PositionsTotal();
   int count = 0;
   ...
   for(int i=0; i<total; i++)
   {
      ...
      if(StringFind(comment, "FOL") < 0) // Exclude Follow
         count++;
   }
   return count;
}
...
void CornerInfoUpdate()
{
   ...
   // --- [NEW] Safety & Friday Info ---
   string v7 = (g_isMarketSafe ? "OK" : "PAUSED");
   color  c7 = (g_isMarketSafe ? clrLime : clrRed);
   string t7 = "Safety Stat";

   string v8 = g_safetyReason;
   string t8 = "Safety Reason";

   double dayClosedProfit = ClosedProfitSince(DayStart(TimeCurrent()));
   double floatingProfit  = BasketProfitByMagicSide(g_magicBuy, POSITION_TYPE_BUY) + 
                             BasketProfitByMagicSide(g_magicSell, POSITION_TYPE_SELL);
   string v9 = StringFormat("%.1f", dayClosedProfit + floatingProfit);
   string t9 = "Friday Net";

   CornerLabel(g_dashPrefix+"CI_V7", g_cornerCorner, g_cornerValueX, baseY - g_cornerLineGap*1, v7, c7, g_cornerFontBase, ANCHOR_RIGHT_LOWER);
   CornerLabel(g_dashPrefix+"CI_T7", g_cornerCorner, g_cornerTextX,  baseY - g_cornerLineGap*1, t7, g_cornerTextColor,  g_cornerFontBase, ANCHOR_RIGHT_LOWER);

   CornerLabel(g_dashPrefix+"CI_V8", g_cornerCorner, g_cornerValueX, baseY - g_cornerLineGap*2, v8, g_cornerValueColor, g_cornerFontBase, ANCHOR_RIGHT_LOWER);
   CornerLabel(g_dashPrefix+"CI_T8", g_cornerCorner, g_cornerTextX,  baseY - g_cornerLineGap*2, t8, g_cornerTextColor,  g_cornerFontBase, ANCHOR_RIGHT_LOWER);

   CornerLabel(g_dashPrefix+"CI_V9", g_cornerCorner, g_cornerValueX, baseY - g_cornerLineGap*3, v9, g_cornerValueColor, g_cornerFontBase, ANCHOR_RIGHT_LOWER);
   CornerLabel(g_dashPrefix+"CI_T9", g_cornerCorner, g_cornerTextX,  baseY - g_cornerLineGap*3, t9, g_cornerTextColor,  g_cornerFontBase, ANCHOR_RIGHT_LOWER);
}
// === Remove === */
...
void FridayPlanUpdate()
{
   ...
   // Transition to MANAGE (8-hour rule)
   if(activeDay && g_friState == EZ_STATE_NORMAL)
   {
      double dayProfit = ClosedProfitSince(DayStart(now));
      if(dayProfit >= InpFridayProfitTargetMoney || ttc <= (G_FriStartManage_Hours + addH) * 3600)
      {
         g_friState = EZ_STATE_FRI_MANAGE;
         g_friBaseDD = GetCurrentDDPercent(); // Snapshot the baseline DD%
         g_friRescueBlocked = false;
         Log(StringFormat("[%s] Start MANAGE Phase (Entry Blocked). BaseDD=%.2f%%", (early ? "ECL" : "FRI"), g_friBaseDD));
      }
   }

   // [GROWTH GUARD] Monitor DD growth during Friday
   if(activeDay && (g_friState == EZ_STATE_FRI_MANAGE || g_friState == EZ_STATE_FRI_SOFT))
   {
      double currentDD = GetCurrentDDPercent();
      if(!g_friRescueBlocked && currentDD > (g_friBaseDD + InpFriRescueGrowthLimit_Pct))
      {
         g_friRescueBlocked = true;
         Log(StringFormat("[FRI] RESCUE STOP: DD Growth Limit exceeded (Current=%.2f%%, Base=%.2f%%, Limit=+%.2f%%)", 
                          currentDD, g_friBaseDD, InpFriRescueGrowthLimit_Pct));
      }
   }

   // [PROFIT LOCK] (6-hour rule)
   // Logic: If (Today's Profit) > (Current Floating Loss), close ALL to end the day green.
   if(activeDay && (g_friState == EZ_STATE_FRI_MANAGE || g_friState == EZ_STATE_FRI_SOFT))
   {
      if(ttc <= (InpFriLockProfitBefore_Hours + addH) * 3600)
      {
         double dayClosedProfit = ClosedProfitSince(DayStart(now));
         double floatingProfit  = BasketProfitByMagicSide(g_magicBuy, POSITION_TYPE_BUY) + 
                                  BasketProfitByMagicSide(g_magicSell, POSITION_TYPE_SELL);
         
         if(floatingProfit < 0 && MathAbs(floatingProfit) < dayClosedProfit)
         {
            Log(StringFormat("[FRI] Lock Profit at 6h mark. Floating=%.2f, DayClosed=%.2f.", floatingProfit, dayClosedProfit));
            CloseAllByMagicSideTagged("LOK", g_magicBuy, POSITION_TYPE_BUY);
            CloseAllByMagicSideTagged("LOK", g_magicSell, POSITION_TYPE_SELL);
         }
      }
   }

   // Transition to SOFT (4-hour rule)
   if(activeDay && g_friState == EZ_STATE_FRI_MANAGE)
   {
      if(ttc <= (G_FriSoftClose_Hours + addH) * 3600)
      {
         g_friState = EZ_STATE_FRI_SOFT;
         g_liquidatingBuy  = (CountPositionsByMagicSide(g_magicBuy, POSITION_TYPE_BUY) > 0);
         g_liquidatingSell = (CountPositionsByMagicSide(g_magicSell, POSITION_TYPE_SELL) > 0);
         Log(StringFormat("[%s] Enter SOFT liquidation (No Entries/Rescues)", (early ? "ECL" : "FRI")));
      }
   }

   // Transition to FORCE (1-hour rule)
   if(activeDay && g_friState == EZ_STATE_FRI_SOFT)
   {
      if(ttc <= (G_FriForceClose_Hours + addH) * 3600)
      {
   ...
   if(activeDay && g_friState == EZ_STATE_FRI_SOFT)  SoftLiquidateStep();
   if(activeDay && g_friState == EZ_STATE_FRI_FORCE)
   {
      CloseAllByMagicSideTagged("FRC", g_magicBuy, POSITION_TYPE_BUY);
      CloseAllByMagicSideTagged("FRC", g_magicSell, POSITION_TYPE_SELL);
   }

   // Check if all clear
   if(activeDay && (g_friState == EZ_STATE_FRI_MANAGE || g_friState == EZ_STATE_FRI_SOFT || g_friState == EZ_STATE_FRI_FORCE))
   {
      if(CountPositionsByMagicSide(g_magicBuy, POSITION_TYPE_BUY) == 0 && 
         CountPositionsByMagicSide(g_magicSell, POSITION_TYPE_SELL) == 0)
      {
         if(fri)   g_friState = EZ_STATE_WEEKEND_PAUSE;
         else      g_friState = EZ_STATE_EARLYDAY_PAUSE;
         Log(StringFormat("[%s] Market Clean => PAUSE", (fri ? "FRI" : "ECL")));
      }
   }
}
...
//==================================================================//
// [950] CIRCUIT BREAKER - Global Equity Protection                  //
//==================================================================//
void UpdateCircuitBreaker()
{
   if(!InpEnableBreaker)
   {
      g_breakerRescueBlocked = false;
      g_breakerLevel2Start   = 0;
      return;
   }
   ...
   //--- Level 1: Soft Stop (Stop Rescues)
   if(currentDD >= InpBreakerLevel1_Pct)
   {
      if(!g_breakerRescueBlocked)
      {
         g_breakerRescueBlocked = true;
         Log(StringFormat("[BREAKER] Level 1 hit (%.2f%%). Rescues halted.", currentDD));
      }
   }
   else
   {
   ...
   //--- Level 2: Hard Stop (Close All)
   if(currentDD >= InpBreakerLevel2_Pct)
   {
      if(g_breakerLevel2Start == 0) g_breakerLevel2Start = now;
      
      int elapsed = (int)(now - g_breakerLevel2Start);
      if(elapsed >= InpBreakerVerify_Sec)
      {
         Log(StringFormat("[BREAKER] Level 2 Verified (%.2f%% for %ds). CLOSING ALL.", currentDD, elapsed));
         
         // Hard liquidation
         CloseAllByMagicSideTagged("BRK", g_magicBuy, POSITION_TYPE_BUY);
         CloseAllByMagicSideTagged("BRK", g_magicSell, POSITION_TYPE_SELL);
         
         // Set Cooldown
         g_globalCooldownUntil = now + (InpBreakerCooldown_Min * 60);
         g_breakerLevel2Start = 0;
         g_breakerRescueBlocked = false; 
      }
   }
...
//==================================================================//
// [960] MARKET SAFETY GATES - Volatility & Spread Screening         //
//==================================================================//
bool CheckMarketSafety()
{
   datetime now = TimeCurrent();
   
   // 1. Velocity (M5) - How far has price moved from candle open?
   ...
   // 2. Relative ATR (Current candle vs Avg Volatility)
   double atr = 0;
   double aVal = 0;
   if(GetBufferValue(g_hAtrGrid, 0, atr) && atr > 0)
   {
      double candleRange = (iHigh(_Symbol, PERIOD_CURRENT, 0) - iLow(_Symbol, PERIOD_CURRENT, 0));
      aVal = candleRange / atr;
   }

   // 3. Spread
   int sVal = (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);

   // 4. News Window Logic (String Parser)
   bool newsActive = false;
   string nRange = InpGateNewsRange;
   StringTrimLeft(nRange); StringTrimRight(nRange);
   
   if(nRange != "")
   {
      // Target Format: "YYYY.MM.DD HH:MM-HH:MM"
      string parts[];
      if(StringSplit(nRange, ' ', parts) == 2)
      {
         string datePart = parts[0];
         string timePart = parts[1];
         string tSteps[];
         if(StringSplit(timePart, '-', tSteps) == 2)
         {
            datetime nDay   = StringToTime(datePart);
            datetime nStart = nDay + (datetime)StringToTime("1970.01.01 " + tSteps[0]);
            datetime nEnd   = nDay + (datetime)StringToTime("1970.01.01 " + tSteps[1]);
            if(now >= nStart && now <= nEnd) newsActive = true;
         }
      }
   }
   ...
   string newsStr = (newsActive ? "BLOCK" : "OK");
   if(nRange == "") newsStr = "OFF";
   else if(!InpEnableMarketGates && newsActive) newsStr = "NEWS!"; // Indicator only

   g_marketGateStatus = StringFormat("Gate V:%.0f/%d | A:%.1f/%.1f | S:%d/%d | N:%s", 
                                     vVal, G_GATE_VELOCITY_LIMIT, 
                                     aVal, G_GATE_ATR_RATIO_LIMIT, 
                                     sVal, G_GATE_SPREAD_LIMIT,
                                     newsStr);

   // Blocking Logic - ONLY if enabled
   if(InpEnableMarketGates)
   {
      if(vVal >= G_GATE_VELOCITY_LIMIT || aVal >= G_GATE_ATR_RATIO_LIMIT || sVal >= G_GATE_SPREAD_LIMIT || newsActive)
      {
         g_marketGateBlocked = true;
         return false;
      }
   }

   g_marketGateBlocked = false;
   return true;
}
...
void TryEntry()
{
   ...
   // [NEW] 3. Market Safety Gates Block
   if(g_marketGateBlocked)
      return;

   // 4. Friday Block (8h rule)
   if(g_friState >= EZ_STATE_FRI_MANAGE)
      return;
...
void TryRescue()
{
   ...
   // 2. Circuit Breaker Block (Rescue stops at Level 1)
   if(g_breakerRescueBlocked || g_marketGateBlocked)
      return;
            ...
            if(price < newestOpen)
            {
               ...
               if(GetBufferValue(g_hAtrGrid, 0, atr))
               {
                  ...
                  if(!HasNearbyOpenPrice(g_magicBuy, POSITION_TYPE_BUY, gridPips) && OncePerBar(g_lastRescueBuyBar))
                  {
                     ...
                     {
                        Print("ABRG: Buy Rescue blocked (Hibernating or waiting Gap)");
                     }
                  }
               }
            }
            ...
            if(price > newestOpen)
            {
               ...
               if(GetBufferValue(g_hAtrGrid, 0, atr))
               {
                  ...
                  if(!HasNearbyOpenPrice(g_magicSell, POSITION_TYPE_SELL, gridPips) && OncePerBar(g_lastRescueSellBar))
                  {
                     ...
                     {
                        Print("ABRG: Sell Rescue blocked (Hibernating or waiting Gap)");
                     }
                  }
               }
            }
...
void OnTimer()
{
   // [ASYNC BOOT] Continue the background initialization if not ready
   if(g_bootState != BOOT_READY)
   {
      PerformAsyncBootStep();
      return; 
   }
...
int OnInit()
{
   ...
   // This prevents MT5 from freezing the UI for 60 seconds.
   g_bootState = BOOT_CLEANUP;
   
   // Use high-speed timer for the boot sequence
   EventSetMillisecondTimer(100); 

   return INIT_SUCCEEDED;
}

// Perform one step of the background boot sequence
void PerformAsyncBootStep()
{
   switch(g_bootState)
   {
      case BOOT_CLEANUP:
         Print("EA Boot [1/4]: Initializing Monitor Shell...");
         UpdateBootStatus("1/4 UI");
      ...
      case BOOT_LICENSE:
         Print("EA Boot [2/4]: Verifying License...");
         UpdateBootStatus("2/4 LIC");
         if(!CheckEaezeLicense())
         {
            Print("EAEZE: Initial License Verification Failed.");
            ExpertRemove();
         }
         RemoveLicenseAlert();
         g_bootState = BOOT_INDICATORS;
         break;

      case BOOT_INDICATORS:
         Print("EA Boot [3/4]: Initializing Indicators...");
         UpdateBootStatus("3/4 IND");
         ...
         g_bootState = BOOT_FINALIZING;
         break;

      case BOOT_FINALIZING:
         Print("EA Boot [4/4]: Finalizing Settings...");
         UpdateBootStatus("4/4 FIN");
         
         // Initialize basket state (mid-basket recovery)
         ...
         if(StringLen(InpEarlyCloseDate) > 0) InitEarlyCloseDate();

         Print("==== BOOT SEQUENCE COMPLETE ====");
         g_eae_sync_status = "[OK]";
         g_bootState = BOOT_READY;
         
         // Switch to regular 1-second timer for dashboard updates
         EventKillTimer();
         EventSetTimer(1);
         break;
...
void OnTick()
{
   ...
   // This saves 90% CPU cycles during news/high-tick events while keeping pro-grade protection.
   if(now64 - g_lastPulse64 >= 250)
   {
   ...
   // [EAEZE] Periodic License Verification (every 15 mins)
   CheckEaezeLicensePeriodic();
   if(!G_IsLicenseVerified) return;
   
   IsNewBar();

   // [REMOVED] Safety Gates V1.2.2 calls stripped for clean baseline revert.
   g_isMarketSafe = true;
   g_safetyReason = "OK";
   ...
   bool allowEntryFollow = (g_friState == EZ_STATE_NORMAL) && CanOpenNewOrdersNow() && g_isMarketSafe;
   bool allowRescue      = (g_friState == EZ_STATE_NORMAL || g_friState == EZ_STATE_FRI_MANAGE) && CanOpenNewOrdersNow() && g_isMarketSafe;
   ...
   BasketStateUpdate();
   // EG_Farming_MonitorOnTick(); // [REMOVED] Moved to Timer to ensure non-intrusive processing
}
